...
read_user_input
  ...
  lda #251  ;$fb latch value for port 1
  ...
  lda #%01111111  ;$7f check column 7 where SPACE and Q are located
  ...
  and #%00010000  ;Check for SPACE key
  bne .try_Q_key
  lda #KEY_MASK_PAUSE  ;SPACE pressed so set bit 5 for pause
  bne .save_key_press  ;always branch
.try_Q_key
  lda _KEYBOARD_LATCH	 ;Read answer
  and #%01000000  ;Check for Q key
  bne .end_key_press
  lda #KEY_MASK_ESCAPE  ;Q pressed so set bit 4 for quit
.save_key_press
  sta key_press
.end_key_press
  rts
...
.play_note_fx  ;Y is input into this routine, used to get the sound to play
  ...
  iny
  lda _VOICE_1_HIGH
  and #%11111100  ;mask off 2 low bits
